...
void OpenClEngine::add(const Matrix& A, const Matrix& B, Matrix& C)
{
    ...
    const ulong M = A.rows();
    const ulong N = A.cols();
    // Resize C
    C.resize(M, N);
    ...
    // 2. Create the Kernel object, passing the GPU buffers and dimensions.
    Kernel kernel(m_device, static_cast<ulong>(M) * N, "add_kernel",
                  A_dev, B_dev, C_dev);
...
// --- GPU SCALE ---
void OpenClEngine::scale(Matrix& A, const float scalar_value)
{
    // 1. Allocate memory on the GPU and copy the data from A
    Memory<float> A_dev(m_device, A.size(), 1, const_cast<float*>(A.data()));

    // 2. Create the Kernel object, passing the GPU buffers and dimensions.
    Kernel kernel(m_device, (ulong)A.size(), "scale_kernel",
                  A_dev, scalar_value);
    ...
    // 4. Read the result from the GPU back to the host.
    A_dev.read_from_device();

    // 5. Copy the result to the output Eigen matrix.
    memcpy(A.data(), A_dev.data(), A.size() * sizeof(float));
}
